...
export function ConfigPage() {
  ...
  async function uploadLumoAuth() {
    if (!lumoAuthFile) {
      ...
      setLumoAuthStatus("Select the auth file generated by scripts/generate_lumo_auth.js first.");
      return;
    }
    ...
    try {
      const lumoResult = await postFormData<LumoAuthUploadResponse>("/api/lumo/auth", form);
      if (lumoResult.restartOk) {
        await new Promise((resolve) => setTimeout(resolve, 5000));
        const testOk = await runLumoTest();
        setLumoAuthTone(testOk ? "success" : "warning");
        setLumoAuthStatus(
          testOk
            ? `Auth file uploaded, Lumo restarted, and connectivity test passed using ${lumoResult.filename}.`
            : `Auth file uploaded and Lumo restarted using ${lumoResult.filename}, but the follow-up Lumo test failed. Review the test output below.`
        );
      } else {
        setLumoAuthTone("warning");
        setLumoAuthStatus(
          `Auth file uploaded, but Lumo restart needs attention: ${lumoResult.restartError ?? "unknown restart failure"}. If supervisorctl is unavailable, use the repair action to restart the container, then run Lumo Test again.`
        );
      }
      ...
      const msg = e instanceof Error ? e.message : "unknown error";
      setLumoAuthTone("error");
      setLumoAuthStatus(`Failed to upload Lumo auth file: ${msg}`);
    } finally {
  ...
  return (
    <section className="panel">
      ...
      <label>
        <div>Upload Lumo auth from scripts/generate_lumo_auth.js</div>
      ...
      <button type="button" onClick={uploadLumoAuth} disabled={lumoAuthBusy}>
        {lumoAuthBusy ? "Uploading Lumo auth..." : "Upload Lumo Auth"}
      </button>
      {lumoAuth ? (
